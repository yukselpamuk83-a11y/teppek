// DENEYSEL PROJE - Modern App with Advanced Features
...
function ModernApp() {
  // Router state
  const { currentRoute, isAuthCallback } = useRouter()
  ...
  useEffect(() => {
    const measurePageLoad = speedInsights.measurePageLoad()
    
    analytics.events.pageView('modern_app')
  ...
  useEffect(() => {
    navigator.geolocation.getCurrentPosition(
      (position) => {
        ...
        analytics.track('location_acquired', { 
          method: 'geolocation',
      ...
      (error) => {
        ...
        analytics.track('location_fallback', { reason: error.message })
      }
    )
  }, [])
